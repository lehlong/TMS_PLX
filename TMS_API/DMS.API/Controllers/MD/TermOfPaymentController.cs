...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class TermOfPaymentController(ITermOfPaymentService service) : ControllerBase
    {
        public readonly ITermOfPaymentService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] TermOfPaymentDto TermOfPayment)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(TermOfPayment);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] TermOfPaymentDto TermOfPayment)
        {
            var transferObject = new TransferObject();
            await _service.Update(TermOfPayment);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
