...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class GroupMailController(IGroupMailService service) : ControllerBase
    {
        public readonly IGroupMailService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] GroupMailDto GroupMail)
        {
            var transferObject = new TransferObject();
            GroupMail.Code = Guid.NewGuid().ToString();
            var result = await _service.Add(GroupMail);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] GroupMailDto GroupMail)
        {
            var transferObject = new TransferObject();
            await _service.Update(GroupMail);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
