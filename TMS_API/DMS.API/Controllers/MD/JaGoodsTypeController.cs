...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class JaGoodsTypeController(IJaGoodsTypeService service) : ControllerBase
    {
        public readonly IJaGoodsTypeService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] JaGoodsTypeDto JaGoodsType)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(JaGoodsType);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] JaGoodsTypeDto JaGoodsType)
        {
            var transferObject = new TransferObject();
            await _service.Update(JaGoodsType);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
