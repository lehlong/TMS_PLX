...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class DeliveryGroupController(IDeliveryGroupService service) : ControllerBase
    {
        public readonly IDeliveryGroupService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] DeliveryGroupDto DeliveryGroup)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(DeliveryGroup);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] DeliveryGroupDto DeliveryGroup)
        {
            var transferObject = new TransferObject();
            await _service.Update(DeliveryGroup);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
