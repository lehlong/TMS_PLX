...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class JaDiscountController(IJaDiscountService service) : ControllerBase
    {
        public readonly IJaDiscountService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] JaDiscountDto JaDiscount)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(JaDiscount);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] JaDiscountDto JaDiscount)
        {
            var transferObject = new TransferObject();
            await _service.Update(JaDiscount);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
