...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class JaPtPhanPhoiController(IJaPtPhanPhoiService service) : ControllerBase
    {
        public readonly IJaPtPhanPhoiService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] JaPtPhanPhoiDto JaPtPhanPhoi)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(JaPtPhanPhoi);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] JaPtPhanPhoiDto JaPtPhanPhoi)
        {
            var transferObject = new TransferObject();
            await _service.Update(JaPtPhanPhoi);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
