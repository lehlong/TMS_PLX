...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class JanaPriceController(IJanaPriceService service) : ControllerBase
    {
        public readonly IJanaPriceService _service = service;
        [HttpGet("Search")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] JanaPriceDto JanaPrice)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(JanaPrice);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] JanaPriceDto JanaPrice)
        {
            var transferObject = new TransferObject();
            await _service.Update(JanaPrice);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
