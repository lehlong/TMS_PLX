...
@Component({
  ...
  styleUrl: './config-mail.component.scss',
})
export class ConfigMailComponent {
  ...
  constructor(
    private _service: ConfigMailService,
  ...
  search() {
    this.isSubmit = false
    this._service.searchConfigMail(this.filter).subscribe({
      next: (data) => {
  ...
  submitForm(): void {
    ...
    if (this.validateForm.valid) {
      ...
      if (this.edit) {
        this._service.updateConfigMail(formData).subscribe({
          next: (data) => {
            this.search()
          },
          error: (response) => {
        ...
        this._service.createConfigMail(formData).subscribe({
          next: (data) => {
            this.search()
          },
          error: (response) => {
  ...
  deleteItem(code: string | number) {
    this._service.deleteConfigMail(code).subscribe({
      next: (data) => {
        this.search()
      },
      error: (response) => {
